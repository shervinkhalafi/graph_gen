...
Parsing functions (``parse_stage``, ``parse_architecture``,
``parse_model_type``, ``parse_run_name_fields``, ``parse_protocol``,
``enrich_dataframe``) are imported from the canonical source at
``tmgg.analysis.parsing``.
...
from tmgg.analysis.parsing import enrich_dataframe, parse_protocol
