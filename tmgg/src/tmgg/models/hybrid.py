...
from .base import BaseModel, EmbeddingModel, DenoisingModel
from .gnn import GNN
...
class SequentialDenoisingModel(DenoisingModel):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.denoising_model is not None:
            ...
            Z_denoised, _ = self.denoising_model(Z)
            Z_pred = Z + Z_denoised
        else:
            Z_pred = Z
...
def create_sequential_model(gnn_config: Dict[str, Any], 
                           transformer_config: Optional[Dict[str, Any]] = None) -> SequentialDenoisingModel:
    """
