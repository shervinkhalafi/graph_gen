    ...
    def forward(
        ...
        if self._use_spectral_projections and self.eigen_layer is not None:
            ...
            if actual_k < self._spectral_k:
                ...
                spectral_V = torch.nn.functional.pad(V_tmp, (0, pad_size))  # pyright: ignore[reportAttributeAccessIssue]  # F.pad exists at runtime
                spectral_Lambda = torch.nn.functional.pad(Lambda_tmp, (0, pad_size))  # pyright: ignore[reportAttributeAccessIssue]  # F.pad exists at runtime
            else:
                spectral_V = V_tmp
...
class GraphTransformer(DenoisingModel):
    ...
    @override
    def forward(self, A: torch.Tensor, t: torch.Tensor | None = None) -> torch.Tensor:  # pyright: ignore[reportIncompatibleMethodOverride]  # uses 'A' per domain convention
