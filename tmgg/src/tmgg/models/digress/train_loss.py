...
from tmgg.models.digress.diffusion_utils import mask_distributions
...
class TrainLossDiscrete:
    ...
    def __call__(
        ...
        loss_X = -torch.sum(true_X * torch.log(pred_X), dim=-1)  # (bs, n)
        loss_E = -torch.sum(true_E * torch.log(pred_E), dim=-1)  # (bs, n, n)
