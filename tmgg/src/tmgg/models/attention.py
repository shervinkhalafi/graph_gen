...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, d_k: Optional[int] = None, 
                 d_v: Optional[int] = None, dropout: float = 0.0, bias: bool = False):
        """
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None, 
        ...
        attn_weights_permuted = attn_weights.permute(0, 2, 3, 1)  # (batch_size, seq_len, seq_len, num_heads)
        combined_scores = self.score_combination(attn_weights_permuted).squeeze(-1)  # (batch_size, seq_len, seq_len)
...
class MultiLayerAttention(DenoisingModel):
    ...
    def __init__(self, d_model: int, num_heads: int, num_layers: int, 
        ...
        self.layers = nn.ModuleList([
            MultiHeadAttention(d_model, num_heads=num_heads, d_k=self.d_k, 
                             d_v=self.d_v, dropout=dropout, bias=bias)
