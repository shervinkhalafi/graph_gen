...
Provides logger creation, metric logging, figure logging, S3 sync,
...
@dataclass
class _S3SyncInfo:
    """Associates a TensorBoardLogger with the S3 path it should sync to."""

    logger: TensorBoardLogger
    s3_path: str


_s3_sync_registry: list[_S3SyncInfo] = []
...
def create_loggers(config: DictConfig) -> list[Logger]:
    ...
    if _has_s3_credentials():
        ...
        if not has_s3_tensorboard:
            ...
            # Register S3 destination for sync_tensorboard_to_s3()
            _s3_sync_registry.append(
                _S3SyncInfo(logger=auto_tb_logger, s3_path=s3_path)
            )
...
def sync_tensorboard_to_s3(loggers: list[Logger]) -> None:
    """Sync registered TensorBoard loggers to their S3 destinations.

    Iterates over the ``_s3_sync_registry`` populated by ``create_loggers``
    and uploads each logger's local directory to S3. Called once at the end
    of training so that S3 writes don't slow the training loop.
    """
    import fsspec

    for info in _s3_sync_registry:
        local_path = Path(info.logger.log_dir)

        if not local_path.exists():
            loguru.warning(f"TensorBoard log dir not found: {local_path}")
            continue

        loguru.info(f"Syncing TensorBoard logs to S3: {local_path} -> {info.s3_path}")
        fs = fsspec.filesystem("s3")
        uploaded = 0
        for local_file in local_path.rglob("*"):
            if local_file.is_file():
                rel_path = local_file.relative_to(local_path)
                s3_file = f"{info.s3_path}/{rel_path}"
                fs.put_file(str(local_file), s3_file)
                uploaded += 1
        loguru.info(f"TensorBoard S3 sync complete: {uploaded} files -> {info.s3_path}")
