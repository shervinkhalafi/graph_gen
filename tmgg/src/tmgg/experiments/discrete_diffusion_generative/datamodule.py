...
class SyntheticCategoricalDataModule(BaseGraphDataModule):
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        # 1. Generate adjacency matrices (from BaseGraphDataModule)
        adjacencies_np = self._generate_adjacencies()

        # 2. Split indices (from BaseGraphDataModule)
        train_idx, val_idx, test_idx = self._split_indices(
            len(adjacencies_np), self.train_ratio, self.val_ratio, self.seed
        )
