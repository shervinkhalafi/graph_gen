...
def main(config: DictConfig) -> Dict[str, Any]:
    ...
    if best_model_path:
        best_model = HybridDenoisingLightningModule.load_from_checkpoint(
            best_model_path
        )
...
def evaluate_across_noise_levels(
    model: HybridDenoisingLightningModule,
    ...
    with torch.no_grad():
        for eps in noise_levels:
            ...
            # Move to model's device and ensure double precision
            A_noisy_input = A_noisy_input.double().to(model.device)
