...
from .lightning_module import HybridDenoisingLightningModule
...
@hydra.main(version_base="1.3", config_path="../config", config_name="experiment/default")
def main(config: DictConfig) -> Dict[str, Any]:
    ...
    if best_model_path:
        best_model = HybridDenoisingLightningModule.load_from_checkpoint(best_model_path)
        
        # Perform final evaluation across noise levels
        final_results = evaluate_across_noise_levels(
            best_model, data_module, config.evaluation.noise_levels
        )
        
        # Log final results
        if logger:
            for metric_name, values in final_results.items():
                for i, eps in enumerate(config.evaluation.noise_levels):
...
def evaluate_across_noise_levels(model: HybridDenoisingLightningModule,
                                data_module: GraphDataModule,
                                noise_levels: list) -> Dict[str, list]:
    """
    ...
    with torch.no_grad():
        for eps in noise_levels:
            ...
            A_pred = model(A_noisy_input.double())
            
            # Compute metrics
            metrics = compute_reconstruction_metrics(sample_A, A_pred.squeeze(0))
