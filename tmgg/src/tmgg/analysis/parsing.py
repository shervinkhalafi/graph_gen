...
and flattened config columns.  Every analysis script and notebook should
import from here rather than re-implementing these patterns.

Extracted from ``wandb-tools/aggregate_runs.py`` (primary source) with
supplementary ``parse_dataset`` from ``scripts/unified_arch_comparison.py``
and ``scripts/semantic_analysis.py``.
"""
