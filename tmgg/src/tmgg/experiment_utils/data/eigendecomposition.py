...
def compute_top_k_eigendecomposition(
    ...
    if A.dim() == 2:
        ...
        top_eigenvectors = eigenvectors[batch_indices.unsqueeze(2), :, indices.unsqueeze(1)]
...
def compute_spectral_distance(
    A1: Union[torch.Tensor, np.ndarray],
    A2: Union[torch.Tensor, np.ndarray],
    ...
    if metric == "frobenius":
        ...
        if A1.dim() == 2:
            ...
            return torch.norm(torch.eye(projection.shape[0]) - projection @ projection.T, p='fro')
        else:
            # Batch case
            projection = torch.bmm(eigenvectors1.transpose(-2, -1), eigenvectors2)
            identity = torch.eye(projection.shape[-1]).unsqueeze(0).expand_as(projection)
            return torch.norm(
