...
def _safe_eigsh(A: np.ndarray, k: int) -> tuple:
    ...
    -------
    tuple
        (eigenvalues, eigenvectors) arrays.
    """
    try:
        return eigsh(A, k=k, which="LM", maxiter=10000)
...
def compute_eigenvalue_error(
    ...
    l_true, _ = _safe_eigsh(A_true, k=k)
    l_pred, _ = _safe_eigsh(A_pred, k=k)

    # Guard against division by near-zero norm (e.g., graphs with mostly isolated nodes)
    norm_true = np.linalg.norm(l_true)
