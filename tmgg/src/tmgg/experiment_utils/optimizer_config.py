...
def _configure_cosine_warmup(
    ...
    module
        ...
        training context (max_steps, datamodule, max_epochs).
    optimizer
        ...
        if trainer.max_steps and trainer.max_steps > 0:
            ...
            if dm is not None:
                try:
                    ...
                    warnings.warn(
                        f"Failed to estimate total training steps from datamodule: {exc}. "
                        f"Falling back to default step estimation.",
                        UserWarning,
                        stacklevel=2,
                    )
    ...
    if estimated_total_steps is None:
        raise RuntimeError(
            "Cannot estimate total training steps: trainer.max_steps is not set "
            "and the datamodule is not available. Set max_steps explicitly in the "
            "trainer config or ensure the datamodule is attached."
        )
