"""Dataset classes for graph denoising experiments.
...
# ---------------------------------------------------------------------------
# GraphDataset — PyTorch Dataset for adjacency matrices
# ---------------------------------------------------------------------------
...
class GraphDataset(Dataset[GraphData]):
    ...
    def __getitem__(self, idx: int) -> GraphData | tuple[GraphData, int]:
        """
        Get a sample from the dataset.

        Returns:
            If return_original_idx is False: GraphData from the permuted adjacency matrix
            If return_original_idx is True: (GraphData, original matrix index)
        """
        ...
        if self.return_original_idx:
            return GraphData.from_adjacency(adjacency_matrix), matrix_idx
        else:
            return GraphData.from_adjacency(adjacency_matrix)
