...
def calculate_hybrid_params(
    gnn_layers, gnn_terms, gnn_feat_in, gnn_feat_out, trans_layers, trans_heads
):
    ...
    for num_layers in range(1, 8):
        for num_terms in range(2, 8):
            for feature_dim in [5, 10, 15, 20, 25, 30]:
                params = calculate_gnn_params(
                    num_layers, num_terms, feature_dim, feature_dim
                )
                if abs(params - target_params) <= tolerance:
                    configs["gnn"].append(
                        {
...
def main():
    ...
    print("\n1. ATTENTION MODELS:")
    attention_configs = sorted(
        configs["attention"], key=lambda x: abs(x["params"] - target_params)
    )[:3]
    for cfg in attention_configs:
        print(
            f"   d_model={cfg['d_model']}, heads={cfg['num_heads']}, layers={cfg['num_layers']}"
        )
        print(f"   → {cfg['params']:,} parameters")

    print("\n2. GNN MODELS:")
    gnn_configs = sorted(
        configs["gnn"], key=lambda x: abs(x["params"] - target_params)
    )[:3]
    for cfg in gnn_configs:
        print(
            f"   layers={cfg['num_layers']}, terms={cfg['num_terms']}, dim={cfg['feature_dim_in']}"
        )
        print(f"   → {cfg['params']:,} parameters")

    print("\n3. HYBRID MODELS:")
    hybrid_configs = sorted(
        configs["hybrid"], key=lambda x: abs(x["params"] - target_params)
    )[:3]
    for cfg in hybrid_configs:
        print(
            f"   GNN: layers={cfg['gnn_layers']}, terms={cfg['gnn_terms']}, dim_in={cfg['gnn_feat_in']}, dim_out={cfg['gnn_feat_out']}"
        )
        print(
            f"   Transformer: layers={cfg['trans_layers']}, heads={cfg['trans_heads']}"
        )
        print(f"   → {cfg['params']:,} parameters")
