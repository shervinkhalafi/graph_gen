...
def run_validation(cfg: ValidationConfig) -> ValidationResult:
    ...
    Builds the model, trains for ``max_steps``, then samples graphs
    ...
    noise_schedule = PredefinedNoiseScheduleDiscrete("cosine", cfg.diffusion_steps)
    ...
    # Train
    # Compute training batches per epoch to set val_check_interval safely.
    # Lightning requires val_check_interval <= training batches when integer.
    dm.setup("fit")
    train_batches = len(dm.train_dataloader())
    val_interval: int | float
    if cfg.val_check_interval > train_batches:
        # Fall back to per-epoch validation
        val_interval = 1.0
    else:
    ...
    # For initial loss, use the first logged value (approximation — we take
    # the epoch-end value which is averaged over all steps)
    train_loss_start = float(logged.get("train/loss_epoch", float("nan")))

    # Better approach: check callback_metrics for the step-level loss
    ...
    # Build reference graphs from validation set
    ...
    with torch.no_grad():
        ...
        samples = module.sample_batch(
            batch_size=min(len(ref_graphs), cfg.eval_num_samples),
            num_nodes=cfg.num_nodes,
        )
    ...
    # For loss decrease, we check if the validation NLL is finite (training
    # is too short in smoke test for reliable loss comparison)
    val_nll = float(logged.get("val/epoch_NLL", float("nan")))
