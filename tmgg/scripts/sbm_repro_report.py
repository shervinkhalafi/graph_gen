...
def write_typst_report(variants: list[VariantData], out: Path, fig_dir: Path) -> Path:
    typ = []
    # Typography: Palatino-style serif body (P052 is the GhostScript
    # Palatino clone, ships in the gsfonts package). Fall back through
    # related Palatino-family names if P052 is missing on a given host
    # so the script doesn't choke on rendering. Mono spans use DejaVu
    # Sans Mono; sans heading lifts paired serif/sans contrast similar
    # to the DiGress paper's typographic style.
    typ.append('#set page(paper: "a4", margin: 1.8cm)')
    typ.append(
        "#set text(\n"
        '  font: ("P052", "TeX Gyre Pagella", "URW Palladio L", "Palatino", "Palatino Linotype", "DejaVu Serif"),\n'
        '  size: 10.5pt, lang: "en",\n'
        ")"
    )
    typ.append('#show raw: set text(font: ("DejaVu Sans Mono",), size: 9.5pt)')
    typ.append(
        "#show heading: set text(\n"
        '  font: ("DejaVu Sans", "Inter", "Helvetica Neue"),\n'
        '  weight: "semibold",\n'
        ")"
    )
    typ.append('#set heading(numbering: "1.")')
    typ.append("#set par(justify: true, leading: 0.7em)")
    typ.append("")
