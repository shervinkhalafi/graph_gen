...
def test_attention_model():
    ...
    model = MultiLayerAttention(
        d_model=20, num_heads=4, num_layers=2, dropout=0.0, bias=True
    )
...
def test_gnn_model():
    ...
    model = GNN(num_layers=2, num_terms=3, feature_dim_in=10, feature_dim_out=10)
    ...
    def print_hierarchy(counts_dict, indent=0):
        for key, value in counts_dict.items():
            if (
                ...
                sub_dict = {
                    k: v for k, v in value.items() if k not in ["total", "self"]
                }
                if sub_dict:
