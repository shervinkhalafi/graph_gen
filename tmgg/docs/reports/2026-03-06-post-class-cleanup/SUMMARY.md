...
|---|--------|----------|----------|---------|-------------|
| M-1 | data | SOC-3 | Separation | `_split.py` | Position ambiguous; sole correct caller is data modules layer; should be in `data_modules/` |
| M-2 | data | INC-2 | Inconsistency | `synthetic_graphs.py`, `pyg_datasets.py` | Neither dataset class inherits `torch.utils.data.Dataset`; `__getitem__` vestigial |
...
| M-7 | data | DC-2 | Dead-Code | `data_module.py:124-126` | ~~RESOLVED (`53dcc3bc`)~~ Removed dead `noise_levels`/`noise_type` attrs from `GraphDataModule` and `SingleGraphDataModule`; `__init__` still accepts params for Hydra compatibility |
...
| M-26 | shared_utils | CO-2 | Cohesion | `run_experiment.py:156-269` | Single function handles seed, run-ID, W&B dedup, dirs, config save, datamodule, model, callbacks, checkpoints, training, testing, S3 sync, W&B cleanup |
...
| M-31 | experiments | EXP-R-5 | Redundancy | `cli.py:528-544`, `execute.py:169-184` | Covariance evolution serialization logic duplicated between CLI and Hydra execute |
| M-32 | experiments | EXP-S-2 | Inconsistency | `evaluate_cli.py:100-101` | `compute_mmd_metrics` imported inline despite top-level imports from same module |
...
| M-36 | experiments | EXP-C-4 | Inconsistency | `spectral_arch_denoising/runner.py:3-7`, `gaussian_diffusion_generative/runner.py:3-7` | Docstrings show `python -m` invocation; CLI entry points (`tmgg-spectral-arch`, `tmgg-gaussian-gen`) are canonical |
| M-37 | experiments | EXP-T-1 | Type-Safety | `embedding_study/execute.py:140` | `_generate_graphs` returns bare `list` instead of `list[torch.Tensor]` |
| M-38 | experiments | EXP-T-3 | Type-Safety | `noised_collector.py`, `analyzer.py` | All comparison methods return `dict[str, Any]`; well-defined schemas documented in docstrings but not typed |
...
| M-40 | experiments | EXP-CD-5 | Config-Drift | `base_config_embedding_study.yaml` | Missing `_cli_cmd` key present in every other base config |
| M-41 | experiments | EXP-CS-1 | Config-Structure | `grid_search_base.yaml:49-61` | Inline `trainer.callbacks` bypasses shared `create_callbacks()` path |
| M-42 | experiments | EXP-CS-2 | Config-Structure | `base_config_discrete_diffusion_generative.yaml:69-81` | Inline logger block duplicates S3 path pattern from `base/logger/default.yaml` |
| M-43 | exp_configs | IE-4 | Interpolation-Error | `base_config_discrete_diffusion_generative.yaml:71` | TensorBoard S3 `save_dir` hard-requires `TMGG_S3_BUCKET` env var; no local fallback |
...
| M-46 | exp_configs | RED-3/cfg | Redundancy | `data/sbm_n100.yaml`, `data/sbm_n200.yaml` | Near-copies of `sbm_default.yaml`; only `num_nodes` and `max_block_size` differ |
| M-47 | exp_configs | RED-4/cfg | Redundancy | `stage/stage3_diversity.yaml`, `stage4_benchmarks.yaml`, `stage5_full.yaml` | Identical optimizer, scheduler, noise_levels blocks repeated |
| M-48 | exp_configs | DC-1 | Dead-Config | `base/logger/grid_wandb.yaml` | Never referenced; `grid_search_base.yaml` duplicates content inline |
| M-49 | exp_configs | DC-2 | Dead-Config | `base/logger/tensorboard_s3.yaml` | Hardcoded `s3://your-bucket/...` placeholder; dangerous if selected via `logger=tensorboard_s3` |
| M-50 | exp_configs | DC-4 | Dead-Config | `data/er_spectral.yaml`, `lfr_spectral.yaml`, `regular_spectral.yaml`, `tree_spectral.yaml` | Four `_spectral` data configs appear unused by any pipeline or defaults list |
| M-51 | exp_configs | HBP-2 | Hydra-Practice | `base_config_discrete_diffusion_generative.yaml:52-82` | Inline callback/logger overrides bypass `base/callbacks/` and `base/logger/` config groups |
...
| N-4 | models | M/T-4 | Type-Safety | `base.py:30` | `parameter_count` returns `dict[str, Any]`; could be `dict[str, int \| dict[str, Any]]` |
| N-5 | experiments | EXP-D-2 | Dead-Code | `eigenstructure_study/analyzer.py:490-513` | `SpectralAnalyzer.save_results` never called; CLI and execute both write JSON directly |
...
| N-7 | experiments | EXP-C-6 | Inconsistency | `stages/runner.py:42` | Lazy-imports `run_experiment` inside `main()` without explanation; all other runners import at module level |
...
| N-10 | exp_configs | DC-5 | Dead-Config | `grid_search_base.yaml:72` | `evaluation.final_eval_noise_levels` read by no Python code |
| N-11 | exp_configs | DC-6 | Dead-Config | `stage/stage3-5` configs | `trigger_conditions`, `ablations`, `statistical_tests` keys read by no code; aspirational documentation |
| N-12 | exp_configs | HBP-1 | Hydra-Practice | various data configs | `# @package data` header present in some, absent in others; Hydra infers it automatically |
