...
- **Quantity plotted**: the **fraction of variance explained** (FVE) of the projected clean adjacency `B = V̂_k^T A V̂_k` by the noisy top-*k* eigenvalues `Λ̃_k`. Formally `FVE = Var(E[B | Λ̃_k]) / Var(B) ∈ [0, 1]`, i.e. the population R² of the Bayes-optimal predictor. The kNN conditional-mean regressor (m=10) that estimates the numerator carries a ≈1/m finite-sample bias; a permutation null (features shuffled) measures that bias, and the **calibrated FVE margin** = `FVE_real − FVE_null` is the bias-corrected signal.
- **Palette**:
