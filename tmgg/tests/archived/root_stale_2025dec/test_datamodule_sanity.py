...
def test_sbm_datamodule():
    ...
    print(
        f"  ✓ Created {len(dm.train_adjacency_matrices)} train, {len(dm.val_adjacency_matrices)} val, {len(dm.test_adjacency_matrices)} test matrices"
    )
    ...
    assert (
        train_batch.shape[0] == 5
    ), f"Expected batch size 5, got {train_batch.shape[0]}"
    assert (
        train_batch.shape[1] == train_batch.shape[2] == 5
    ), "Expected 5x5 adjacency matrices"
...
def test_dataloader_consolidation():
    ...
    ), f"Train should use RandomSampler, got {type(train_loader.sampler)}"
    assert isinstance(
        val_loader.sampler, SequentialSampler
    ), f"Val should use SequentialSampler, got {type(val_loader.sampler)}"
    assert isinstance(
        test_loader.sampler, SequentialSampler
    ), f"Test should use SequentialSampler, got {type(test_loader.sampler)}"
...
def test_config_inheritance():
    ...
    try:
        ...
        with initialize(
            config_path="src/tmgg/exp_configs/base/data", version_base=None
        ):
